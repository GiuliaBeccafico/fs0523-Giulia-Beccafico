...
let risultato1 = whoIsBigger(10, 20);
console.log(risultato1);

let risultato2 = whoIsBigger(8, 40);
...
let risultatoStringa = splitMe('Sto facendo l\'esame');
...
let risultatoDeleteOne1 = deleteOne('Ciao sono Giulia', true);
console.log(risultatoDeleteOne1);
let risultatoDeleteOne2 = deleteOne('Ciao sono Giulia', false)
console.log(risultatoDeleteOne2);

/* ESERCIZIO 5
  Crea una funzione chiamata "onlyLetters" che riceve una stringa 
  come parametro e la ritorna eliminando tutte le cifre numeriche.
...
let stringaNumber = onlyLetters('Ho 2 gatti rossi di nome Simba e Barbero e 1 tartaruga di nome Pilla');
...
console.log(isThisAnEmail("beccaficogiulia@email.com"));
console.log(isThisAnEmail("giuliabeccafico_email.com"));
console.log(isThisAnEmail("giuliabeccafico"));

/* ESERCIZIO 7
  Scrivi una funzione chiamata "whatDayIsIt" 
  che ritorna il giorno della settimana corrente.
*/
...
function howManyDays(data) {
  ...
  var differenzaInGiorni = Math.floor((oggi - dataPassata) / (1000 * 60 * 60 * 24));
  return differenzaInGiorni;
}

let dataPassata = '2023-10-01';
let giorniTrascorsi = howManyDays(dataPassata);
console.log('Numero di giorni trascorsi: ' + giorniTrascorsi);

/* ESERCIZIO 10
  Scrivi una funzione chiamata "isTodayMyBirthday" che deve ritornare true se oggi è il 
  tuo compleanno, falso negli altri casi.
*/

function isTodayMyBirthday(dataDiNascita) {
  let oggi = new Date();
  let giornoDiNascita = dataDiNascita.getDate();
  let meseDiNascita = dataDiNascita.getMonth();
  let giornoCorrente = oggi.getDate();
  let meseCorrente = oggi.getMonth();
  if (giornoDiNascita === giornoCorrente && meseDiNascita === meseCorrente) {
    return true;
  } else {
    return false;
  }
...
let dataDiNascita = new Date(1994, 1, 9);
let compleannoOggi = isTodayMyBirthday(dataDiNascita);
console.log("Oggi è il mio compleanno? " + compleannoOggi);
...
let oggetto = {
  nome: "Giulia",
  età: 29,
  giocoPreferito: 'Hollow Knight',
  filmPreferito: 'Big Fish',
};

function deleteProp(obj, frase) {
  if (obj.hasOwnProperty(frase)) {
    delete obj[frase];
  } return obj;
}

let nuovoOggetto = deleteProp(oggetto, "giocoPreferito");
...
function newestMovie(movies) {
  let filmPiuRecente = movies.reduce(function (filmCorrente, filmSuccessivo) {
    return filmCorrente.year > filmSuccessivo.year ? filmCorrente : filmSuccessivo;
  })
...
function onlyTheYears(movies) {
  let anniUscita = movies.map(function (film) {
    return film.Year;
  });
  return anniUscita;
}
let anniUscita = onlyTheYears(movies)
console.log("La lista degli anni è:", anniUscita)
...
function onlyInLastMillennium(movies) {
  let filmMillennioScorso = movies.filter(function (film) {
    return film.Year >= 1900 && film.Year <= 1999;
  });
...
function sumAllTheYears(movies) {
  const sommaAnni = movies.reduce((acc, movie) => {
...
console.log("La somma degli anni di produzione è: ", somma)
...
function stampaTestoInTdElements() {
...
function aggiungiClasseAlleRighe() {
